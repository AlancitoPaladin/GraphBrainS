...
# ===========================================
directories = [
    'images1/1/Training/glioma',
    'images1/1/Training/meningioma',
    'images1/1/Training/notumor',
    'images1/1/Training/pituitary'
]
...
# ===========================================
# PROCESAMIENTO PARA CADA DIRECTORIO
# ===========================================
for training_path in directories:
    ...
    # Ordenar y tomar solo las primeras 10
    image_files.sort()
    image_files = image_files[:10]

    print(f"\nProcesando las primeras 10 imágenes de {training_path}")
    print(f"Imágenes encontradas: {len(image_files)}")
    print(f"Resultados se guardarán en: {results_path}")

    # Crear subcarpeta para este tipo de tumor
    tumor_type = os.path.basename(training_path)
    tumor_results_path = os.path.join(results_path, tumor_type)
    if not os.path.exists(tumor_results_path):
        os.makedirs(tumor_results_path)
    ...
    for i, filename in enumerate(image_files, 1):
        ...
        try:
            print(f"\n=== Procesando imagen {i}/10: {filename} ===")
            ...
            original_path = os.path.join(tumor_results_path, f"{base_name}_original.jpg")
            ...
            if u is not None:
                ...
                mask_path = os.path.join(tumor_results_path, f"{base_name}_segmentation.jpg")
                cv2.imwrite(mask_path, segmentation_mask)

            # 3. Guardar imagen con contornos
            if img_contours is not None:
                contours_path = os.path.join(tumor_results_path, f"{base_name}_contours.jpg")
            ...
            if otsu_result is not None:
                otsu_path = os.path.join(tumor_results_path, f"{base_name}_otsu.jpg")
                cv2.imwrite(otsu_path, otsu_result)

            # 5. Guardar cavidad craneal (si existe)
            if brain_cavity is not None:
                cavity_path = os.path.join(tumor_results_path, f"{base_name}_brain_cavity.jpg")
                cv2.imwrite(cavity_path, brain_cavity)

            # 6. Guardar datos numéricos
            stats_path = os.path.join(tumor_results_path, f"{base_name}_stats.txt")
    ...
    # ===========================================
    # GUARDAR RESUMEN POR TIPO DE TUMOR
    # ===========================================
    summary_path = os.path.join(tumor_results_path, 'summary.txt')
    with open(summary_path, 'w') as f:
        f.write(f"RESUMEN DE RESULTADOS DE SEGMENTACIÓN PARA {tumor_type.upper()}\n")
        f.write("=" * 50 + "\n\n")
    ...
    print(f"\n=== PROCESO COMPLETADO PARA {tumor_type.upper()} ===")
...
print("\n=== PROCESO COMPLETADO PARA TODOS LOS DIRECTORIOS ===")
